    ...
    updateElements(e, i, a, s) {
        ...
        for (let t = i; t < i + a; ++t) {
            ...
            var y = isNullOrUndef(_[g]),
                k = M[u] = r.getPixelForValue(_[u], t),
                S = M[g] = n || y ? o.getBasePixel() : o.getPixelForValue(l ? this.applyStack(o, _, l) : _[g], t);
            M.skip = isNaN(k) || isNaN(S) || y, M.stop = 0 < t && Math.abs(_[u] - x[u]) > m, f && (M.parsed = _, M.raw = h.data[t]), d && (M.options = c || this.resolveDataElementOptions(t, b.active ? "active" : s)), v || this.updateElement(b, t, M, s), x = _
        }
...
class Scale extends Element {
    ...
    _computeLabelSizes(t, e, i) {
        ...
        let l = 0,
            h = 0,
            c, d, u, g, p, f, m, v, x, b, _;
        for (c = 0; c < e; c += o) {
            if (g = t[c].label, p = this._resolveTickFontOptions(c), a.font = f = p.string, m = s[f] = s[f] || {
                    data: {},
                    gc: []
                }, v = p.lineHeight, x = b = 0, isNullOrUndef(g) || isArray(g)) {
                if (isArray(g))
                    for (d = 0, u = g.length; d < u; ++d) _ = g[d], isNullOrUndef(_) || isArray(_) || (x = _measureText(a, m.data, m.gc, x, _), b += v)
    ...
    _computeGridLineItems(t) {
        ...
        var f, m, u = g / 2;
        let v, x, b, _, y, k, S, M, D, P, w, C;
        "top" === n ? (v = p(this.bottom), k = this.bottom - c, M = v - u, P = p(t.top) + u, C = t.bottom) : "bottom" === n ? (v = p(this.top), P = t.top, C = p(t.bottom) - u, k = v + u, M = this.top + c) : "left" === n ? (v = p(this.right), y = this.right - c, S = v - u, D = p(t.left) + u, w = t.right) : "right" === n ? (v = p(this.left), D = t.left, w = p(t.right) - u, y = v + u, S = this.left + c) : "x" === e ? ("center" === n ? v = p((t.top + t.bottom) / 2 + .5) : isObject(n) && (m = n[f = Object.keys(n)[0]], v = p(this.chart.scales[f].getPixelForValue(m))), P = t.top, C = t.bottom, k = v + u, M = k + c) : "y" === e && ("center" === n ? v = p((t.left + t.right) / 2) : isObject(n) && (m = n[f = Object.keys(n)[0]], v = p(this.chart.scales[f].getPixelForValue(m))), y = v - u, S = y - c, D = t.left, w = t.right);
        ...
        for (x = 0; x < h; x += A) {
            ...
            void 0 !== (b = getPixelForGridLine(this, x, o)) && (_ = _alignPixel(i, b, T), l ? y = S = D = w = _ : k = M = P = C = _, d.push({
                tx1: y,
                ty1: k,
                tx2: S,
                ty2: M,
    ...
    _computeLabelItems(t) {
        ...
        const p = [];
        let f, m, v, x, b, _, y, k, S, M, D, P = "middle";
        ...
        for (f = 0, m = o.length; f < m; ++f) {
            ...
            var C = s.setContext(this.getContext(f)),
                A = (y = this.getPixelForTick(f) + s.labelOffset, S = (k = this._resolveTickFontOptions(f)).lineHeight, (M = isArray(v) ? v.length : 1) / 2),
                O = C.color,
                L = C.textStrokeColor,
                T = C.textStrokeWidth;
            let t = _;
            r ? (x = y, "inner" === _ && (t = f === m - 1 ? this.options.reverse ? "left" : "right" : 0 === f ? this.options.reverse ? "right" : "left" : "center"), D = "top" === a ? "near" === h || 0 != g ? -M * S + S / 2 : "center" === h ? -w.highest.height / 2 - A * S + S : -w.highest.height + S / 2 : "near" === h || 0 != g ? S / 2 : "center" === h ? w.highest.height / 2 - A * S : w.highest.height - M * S, d && (D *= -1), 0 == g || C.showLabelBackdrop || (x += S / 2 * Math.sin(g))) : (b = y, D = (1 - M) * S / 2);
            let i;
...
class Legend extends Element {
    ...
    _draw() {
        ...
        const {
            ...
            boxHeight: P,
            itemHeight: w
        } = getBoxSize(v, k), C = this.isHorizontal(), A = this._computeTitleHeight(), O = (M = C ? {
        ...
        this.legendItems.forEach((t, e) => {
            ...
            let s = M.x,
                n = M.y;
            b.setWidth(this.width), C ? 0 < e && s + i + y > this.right && (n = M.y += O, M.line++, s = M.x = _alignStartEnd(m, this.left + y, this.right - p[M.line])) : 0 < e && n + O > this.bottom && (s = M.x = s + g[M.line].width + y, M.line++, n = M.y = _alignStartEnd(m, this.top + A + y, this.bottom - g[M.line].height));
            var r, o, l, h, c, d, e = b.x(s);
            e = e, h = n, c = t, isNaN(D) || D <= 0 || isNaN(P) || P < 0 || (f.save(), r = valueOrDefault(c.lineWidth, 1), f.fillStyle = valueOrDefault(c.fillStyle, x), f.lineCap = valueOrDefault(c.lineCap, "butt"), f.lineDashOffset = valueOrDefault(c.lineDashOffset, 0), f.lineJoin = valueOrDefault(c.lineJoin, "miter"), f.lineWidth = r, f.strokeStyle = valueOrDefault(c.strokeStyle, x), f.setLineDash(valueOrDefault(c.lineDash, [])), v.usePointStyle ? (o = {
                radius: P * Math.SQRT2 / 2,
                pointStyle: c.pointStyle,
                rotation: c.rotation,
                borderWidth: r
            }, l = b.xPlus(e, D / 2), d = h + S, drawPointLegend(f, o, l, d, v.pointStyleWidth && D)) : (o = h + Math.max((k - P) / 2, 0), l = b.leftForLtr(e, D), d = toTRBLCorners(c.borderRadius), f.beginPath(), Object.values(d).some(t => 0 !== t) ? addRoundedRectPath(f, {
                x: l,
                y: o,
                w: D,
                h: P,
                radius: d
            }) : f.rect(l, o, D, P), f.fill(), 0 !== r && f.stroke()), f.restore()), s = _textX(a, s + D + S, C ? s + i : this.right, u.rtl), h = b.x(s), e = n, c = t, renderText(f, c.text, h, e + w / 2, _, {
                strikethrough: c.hidden,
                textAlign: b.textAlign(c.textAlign)
            }), C ? M.x += i + y : "string" != typeof t.text ? (d = _.lineHeight, M.y += calculateLegendItemHeight(t, d)) : M.y += O
